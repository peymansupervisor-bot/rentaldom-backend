...
router.post('/contact', async (req, res) => {
    ...
    const { data: existingUsers } = await supabase_1.supabase.auth.admin.listUsers();
    ...
    const { data: profile } = await supabase_1.supabase.from('profiles').select('id').eq('id', userId).single();
    if (!profile) {
        ...
        await supabase_1.supabase.from('profiles').update({ display_name: name.trim() }).eq('id', userId);
    }
    ...
    if (existing) {
        ...
        const { data: conv, error: convErr } = await supabase_1.supabase
            .from('conversations')
            .insert({
    ...
    // Insert message
    await supabase_1.supabase.from('app_messages').insert({
    ...
    await supabase_1.supabase.auth.admin.generateLink({
        type: 'magiclink',
        email: normalizedEmail,
    ...
    notifyUser(supabase_1.supabase, listing.landlord_id, {
